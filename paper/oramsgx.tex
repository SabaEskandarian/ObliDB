...
NASDAQ & 3,300 &\texttt{SELECT Symbol FROM NASDAQ WHERE Sector="Technology" AND Last\_Sale$>$100 AND Last\_Sale$<$200} & .037s & .537s & 14.7$\times$ \\ \rule{0pt}{2ex}
...
FAB & 100,000& \texttt{DELETE FROM FAB WHERE class="a" AND id>1 AND id<900 LIMIT 1}& .350s& .404s& 1.2$\times$ \\\rule{0pt}{2ex} 
FAB & 100,000& \texttt{INSERT INTO FAB (id, val, class, text) VALUES (100000, 1000, "a", "You would measure time the measureless and the immeasurable.")}& .077s& .348s& 4.5$\times$ \\\rule{0pt}{2ex} 
CFPB & 107,000& \texttt{SELECT * FROM CFPB WHERE Date\_Received$>=$2013-05-06 AND Date\_Received$<=$2013-05-10 } & 1.45s & 61.79s & 42.7$\times$\\\rule{0pt}{2ex} 
%FLIGHTS & \texttt{SELECT AVG(Price) FROM FLIGHTS WHERE Destination\_Id=12173 GROUP BY Origin\_Id}& 1.01 & .376\\\rule{0pt}{3ex}
FLIGHTS & 250,000&\texttt{SELECT Price FROM FLIGHTS WHERE Destination\_Id=12173} & .765s & 45.13s & 59.0$\times$\\\rule{0pt}{2ex}
USERVISITS & 350,000 & \texttt{SELECT SUBSTR(sourceIP, 1, X), SUM(adRevenue) FROM USERVISITS GROUP BY SUBSTR(sourceIP, 1, 8)}& 3.52s &205.79s & 58.5$\times$\\\rule{0pt}{2ex}
RANKINGS & 360,000 & \texttt{SELECT pageURL, pageRank FROM RANKINGS WHERE pageRank > 1000 }& .109s & 54.19s& 498.6$\times$ \\\rule{0pt}{2ex}
...
FROM
  ...
  ORDER BY totalRevenue DESC LIMIT 1} & 15.20s & 131.30s& 8.6$\times$\\
\end{tabular}
\caption{Comparison of \name/ and a baseline where a naive oblivious database implementation directly ports non-oblivious algorithms to their oblivious counterparts via ORAM (queries on FAB compare against a baseline B+ tree index implementation). Queries executed on real data sets of various sizes, including three realistic queries on real-world data \cite{NASDAQ, CFPB, FLIGHT} and three queries from the Big Data Benchmark \cite{BDB}. \name/ outperforms the baseline on all queries, but the speedup increases as the data size grows and in queries where \name/ can take advantage of its oblivious indexes.}
\label{figQueries}
\end{figure*}
...
\end{tikzpicture}\\
\end{tabular}
\caption{Comparison of Linear and Index versions of various operators over a table of 100,000 rows. Linear scans do better when most of the data needs to be accessed regardless, but Indexed structures perform far better for small queries. Operations involving modification of the database are far faster for indexed structures.}
\label{figC1}
\end{figure*}
...
\begin{axis}[
    ...
    title={Effectiveness of Low-Leakage Optimization}, 
	symbolic x coords={5\% of Table, 95\% of Table, 4},
	legend style={at={(0.5,-0.17)},
...
\addplot 
	coordinates {(5\% of Table, 9.31225) (95\% of Table, 10.69292) (4,1)}; %hash
\addplot 
	coordinates {(5\% of Table, .84819) (95\% of Table, 2.3543) (4,1)};	%small/large
\addplot 
	coordinates {(5\% of Table, 1.69821) (95\% of Table, 1.91123) (4,1)};	%cont

\legend{No Optimization, Low-Leakage Opt., Continuous Opt.}
\end{axis}
\end{tikzpicture}
\caption{Comparison of \texttt{SELECT} algorithms on Linear table where 5 and 95 percent of a 100,000 row table are retrieved. Low-leakage optimizations (small/large or continuous \texttt{SELECT} algorithms) improve performance over no optimization by 4.5-11$\times$.} 
\label{figC3}
\end{figure}
...
\begin{semilogyaxis}[
	...
	ylabel={Time [seconds]},
    title={Comparison to Baseline and Spark SQL},
...
\addplot %spark sql
    ...
    \legend{Baseline Implementation, \name/, Spark SQL (No Security)}
\end{semilogyaxis}
\end{tikzpicture}
\vskip .3cm
\centering
...
\addplot %opaque obliv
	coordinates {(BDB Q1,24.15) (BDB Q2,47.96) (BDB Q3,6.58) (4, 1)};
\addplot %us
	coordinates {(BDB Q1,498.6) (BDB Q2, 58.46) (BDB Q3,8.64) (4,1)};
%\addplot %spark sql
%	coordinates {(BDB Q1,335.54) (BDB Q2,104.16) (BDB Q3,18.12) (4,1)};
\addlegendimage{line legend,black,sharp plot,dashed}
\coordinate (A) at (axis cs:BDB Q1,335.54);
\coordinate (B) at (axis cs:BDB Q2,335.54);
\coordinate (C) at (axis cs:BDB Q2,104.16);
\coordinate (D) at (axis cs:BDB Q3,104.16);
\coordinate (E) at (axis cs:BDB Q3,18.12);
\coordinate (F) at (axis cs:4,18.12);
...
\end{semilogyaxis} 
\end{tikzpicture}
\caption{Comparison of \name/ to a baseline implementation, Opaque Oblivious mode (single node) \cite{ZDB+17}, and Spark SQL \cite{SparkSQL} on the queries of the Big Data Benchmark \cite{BDB} over tables of 360,000 and 350,000 rows provided with the benchmark. Opaque Oblivious mode provides equivalent security guarantees to \name/, and Spark SQL provides no security guarantees. \name/ outperforms the baseline by 9-499$\times$ and Opaque by 1.2-20$\times$. It also never does worse than 2.1$\times$ the performance of Spark SQL, even outperforming Spark SQL on Query 1 where it can make effective use of an oblivious index.}  
\label{figOpaque} 
\end{figure}

We extensively evaluate \name/ on fabricated data (table FAB in \ref{figQueries}) with table sizes of up to 500,000 rows to demonstrate that choices of data structures and algorithms offered by \name/ allow for meaningful optimizations in handling queries without increasing leakage. We also execute real queries on multiple real-world data sets: 
\begin{itemize}[itemsep=0pt,parsep=0pt]
...
We would also like to compare \name/ to the range search scheme of Demertzis et al \cite{DPP+16} which does not use trusted SGX hardware and hides data but does leak access patterns. Unfortunately, no implementation of this scheme is currently available online or from the authors, so the best we can do is to examine the graphs in the original paper and observe that \name/'s performance is comparable to that reported by \cite{DPP+16} on a similarly-sized USPS data set with more powerful hardware despite \name/'s far stricter leakage requirements. 
