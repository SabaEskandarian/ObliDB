...
\maketitle
  \begin{abstract}
{We present \name/\footnote{Much like the data in our system, sloths, who are abundantly present in trees in some parts of the world, are difficult to spot as they are well-hidden amongst the trees where they spend most of their lives.}, a database built on Intel's SGX hardware that provides minimal leakage of cryptographically protected data. \name/ achieves comparable performance to prior work while leaking very little beyond the structure of protected data. In particular, access patterns to data are hidden. \name/ supports a broad range of SQL queries including groupings and joins and makes use of both ORAM and linear search data structures to provide a range of oblivious algorithms to execute SQL operations with different result sizes. 
}
\end{abstract} 
...
\section{Goals and Threat Model}\label{model}
The ultimate goal of \name/ is to prove the feasibility of a database management system with the functionality to support broad classes of applications while making strong security guarantees. We also wish for \name/ to perform as well as other schemes with weaker security properties. We aim to leverage the added power of the assumption of security in SGX to realize the ``more for less'' goal of better security without performance loss. To this end, we assume that the SGX platform is secure and do not directly handle most side-channels known to affect SGX hardware (see section \ref{related}). That said, we note that in many cases our control data structures are small enough to fit in processor registers, and it is likely that \name/ can, without heavy modification, provide security against many well-known side-channel attacks. 

The particular threat model we assume for \name/ is a malicious operating system with power to examine untrusted memory and any communication between the processor and memory. Moreover, the operating system can maliciously schedule processes or interrupt the execution of an enclave. Although we give the operating system the power to maliciously alter the execution of the program, we assume that it faithfully responds to requests for data via OCALLs. This assumption could be removed with additional lightweight safeguards, but we do not fully implement these safeguards. We note that it is always possible for a malicious operating system to launch an indefinite denial of service attack against an enclave, but such an attack does not compromise security and is outside the scope of the security of SGX for our purposes. 

To carefully describe the level of security provided by \name/, we follow the convention \cite{FVY+17} of describing security in terms of the level of leakage in our scheme. The highest bar of leakage is to leak only ``structural'' information about data, queries, and responses, i.e., to only reveal information that can be hidden by padding. This includes, for example, the size of tables in the database, the sizes of queries, and the sizes of responses to queries\footnote{We do not make an effort to hide the number of tables in a database or which table(s) a particular query accesses. Our goals deal only with the security of data within individual tables.}. Hiding this leakage can only be accomplished with padding at a necessarily high performance cost. \name/ comes close to achieving this level of security and can indeed be configured to leak only structural data at the cost of some restriction on the types of permissible queries and algorithms for executing those queries. In its unmodified form, \name/ leaks structural data as well as the following information:
\begin{enumerate}
\item Whether data returned from a query comes from a continuous block in the table being queried

\item Which data structure is being used to store a table

\item If an index is used on a table, the fraction of the table scanned in order to service a query on that index
\end{enumerate}
Further details of how the following leakage properties are achieved can be seen in sections \ref{oblivData} and \ref{oblivOps}, where the leakage of each operator is explained. We point out that item (1) above can be mitigated by not using the ``Continuous'' strategy for \texttt{SELECT} queries with no impact on supported functionality, a performance tradeoff that can be made by individual users. 

Another concern with regards to performance is that, however secure the properties of the database management system, the way it is used by an application interacting with it can leak additional information. For example, if a web application makes a second query to a database based on the results of a first query, observing the size of the response to the second query may leak additional information about the first query or its response. This is a direct, if unexpected, consequence of structural leakage, and it falls on application developers to consider performance goals against the ramifications of such leakage in their design process. 
...
In addition to the sizes of $T$ and $O$, the fact that this algorithm is used over one of the other options leaks the fact that the result set is drawn from a continuous set of rows in the table. Users concerned about this additional leakage could disable this option and use one of the other options. No other information about access patterns leaks, however, because the memory access pattern is fixed: at each step, the algorithm reads the next row of $T$ and then writes to the next row of $O$. 
...
\begin{axis}[
    ...
    coordinates {
    (5,21.0733)(10,22.78638)(15,22.76745)(20,22.52756)
    };
    \addplot[
    color=blue,
    mark=square,
    ...
    coordinates {
    (5,3.91814)(10,7.5947)(15,11.25464)(20,14.93086)
    };
    \legend{Linear, Indexed}
\end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
\begin{axis}[
    title={Sloth vs Baseline: \texttt{SELECT}},
    xlabel={Percent of Table Retrieved},
    ylabel={Time [seconds]},
    xmin=5, xmax=20,
    ymin=0, ymax=40,
