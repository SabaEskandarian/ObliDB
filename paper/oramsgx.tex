...
NASDAQ & 3,300 &\texttt{SELECT Symbol FROM NASDAQ WHERE Sector="Technology" AND Last\_Sale$>$100 AND Last\_Sale$<$200} & 0.037s & 0.537s & 14.7$\times$ \\ \rule{0pt}{2ex}
...
SYNTH & 100,000& \texttt{DELETE FROM SYNTH WHERE class="a" AND id>1 AND id<900 LIMIT 1}& 0.350s& 0.404s& 1.2$\times$ \\\rule{0pt}{2ex} 
SYNTH & 100,000& \texttt{INSERT INTO SYNTH (id, val, class, text) VALUES (100000, 1000, "a", "You would measure time the measureless and the immeasurable.")}& 0.077s& 0.348s& 4.5$\times$ \\\rule{0pt}{2ex} 
...
FLIGHTS & 250,000&\texttt{SELECT Price FROM FLIGHTS WHERE Destination\_Id=12173} & 0.765s & 45.13s & 59.0$\times$\\\rule{0pt}{2ex}
...
RANKINGS & 360,000 & \texttt{SELECT pageURL, pageRank FROM RANKINGS WHERE pageRank > 1000 }& 0.109s & 54.19s& 498.6$\times$ \\\rule{0pt}{2ex}
...
\end{tikzpicture}\\
\end{tabular}
\caption{Comparison of Linear and Index versions of operators over SYNTH table. Linear scans do better when most of the data needs to be accessed, but Indexed structures perform far better for small queries. Operations involving modification of the database are far faster for indexed structures.}
\label{figC1}
\end{figure*}
...
\end{semilogyaxis} 
\end{tikzpicture}
\caption{\name/ outperforms Baseline and Opaque oblivious~\cite{ZDB+17} by 9-499$\times$ and 1.2-20$\times$ respectively and never runs more than 2.1$\times$ slower than Spark SQL~\cite{SparkSQL} on the Q1-Q3 of the Big Data Benchmark~\cite{BDB}.}  
\label{figOpaque} 
\end{figure} 
